...
int test_matrix_mul_fifo_core()
{
   ...
   //convert matrix in input stream. b is sent first!!!
	for(i = 0; i < MAT_DIM; i++) {
		for(j = 0; j < MAT_DIM; j++) {
   ...
   matrixmul_fifo_accel_core(in_stream, out_stream);
   ...
   cout << "Result by HW (For fifo mult mat)" << endl;
...
int main(int argc, char **argv)
{
    ...
    err_cnt = test_matrix_mul_fifo_core();
