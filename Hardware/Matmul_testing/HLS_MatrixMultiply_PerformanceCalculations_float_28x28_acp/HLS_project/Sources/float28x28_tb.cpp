...
int main() {
	...
	float28x28(in_stream, out_stream);
	...
	for (i = 0; i < OUT_C_ROWS; i++) {
		for (j = 0; j < OUT_C_COLS; j++) {
			error += fabsf(testResultMatrix_c[i][j] - referenceResultMatrix_c[i][j]);
			//printf("Error calculation %d_%d (|%d - %d|) = %d\n", i, j, testResultMatrix_c[i][j], referenceResultMatrix_c[i][j], abs(testResultMatrix_c[i][j] - referenceResultMatrix_c[i][j]));
		}
	}
