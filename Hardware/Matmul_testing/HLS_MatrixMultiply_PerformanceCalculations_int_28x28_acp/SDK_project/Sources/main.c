...
// NB: Stack and heap size has to be changed from the default 1KB to 10KB for both
...
// int28x28 SW version
void int28x28(int a[IN_A_ROWS][IN_A_COLS], int b[IN_B_ROWS][IN_B_COLS], int c[OUT_C_ROWS][OUT_C_COLS]) {
...
int main() {
	...
	int status;
	XInt28x28 doInt28x28;
	XInt28x28_Config XInt28x28_config = { 0, XPAR_INT28X28_0_S_AXI_CONTROL_BUS_BASEADDR };
	...
	// Fill test matrices
	int testMatrix_a_SW[IN_A_ROWS][IN_A_COLS];
	int testMatrix_b_SW[IN_B_ROWS][IN_B_COLS];
	int testResultMatrix_c_SW[OUT_C_ROWS][OUT_C_COLS];
	int testResultMatrix_c_HW[OUT_C_ROWS][OUT_C_COLS];
	int referenceResultMatrix_c[OUT_C_ROWS][OUT_C_COLS];
    // Generate test matrices and reference matrix
    for (int i = 0; i < IN_A_ROWS; i++) {
        for (int j = 0; j < IN_A_COLS; j++) {
        	testMatrix_a_SW[i][j] = i+j;
        }
    }
    for (int i = 0; i < IN_B_ROWS; i++) {
        for (int j = 0; j < IN_B_COLS; j++) {
        	testMatrix_b_SW[i][j] = i*j;
        }
    }
	for (int i = 0; i < IN_A_ROWS; i++) {
		for (int j = 0; j < IN_B_COLS; j++) {
	...
	int28x28(testMatrix_a_SW, testMatrix_b_SW, testResultMatrix_c_SW);
	...
	for (int i = 0; i < OUT_C_ROWS; i++) {
		for (int j = 0; j < OUT_C_COLS; j++) {
			if (testResultMatrix_c_SW[i][j] != referenceResultMatrix_c[i][j]) {
				error++;
			}
		}
	}
	...
	status = XInt28x28_CfgInitialize(&doInt28x28, &XInt28x28_config);
	if (status != XST_SUCCESS) {
	...
	XInt28x28_InterruptGlobalDisable(&doInt28x28);
	XInt28x28_InterruptDisable(&doInt28x28, 1);

	XInt28x28_Start(&doInt28x28);
